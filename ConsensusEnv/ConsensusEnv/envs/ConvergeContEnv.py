...
# This file keeps whatever parameters ConsensusContEnv would have (at least prior to 01/06), but
# modifies the problem into a problem of moving towards origin. 
import gym
...
class ConvergeContEnv(gym.Env):
    ...
    def __init__(self, N=5, dt=0.1, v=0.5, v_max=1, boundaries=[-1.6,1.6,-1,1], Delta=0.02, o_radius=0.4, max_iter=200,
        ...
        super(ConvergeContEnv, self).__init__()
    ...
    def step(self, action):
        ...
        # Use -temp_state for consensus. Use -zeros for reaching 0.
        diff = temp_state.T.reshape((self.N,self.ns,1)) - np.zeros((1,self.ns,self.N)) # temp_state.reshape((1,self.ns,self.N))
        ...
        # Use -zeros if the goal is converging to origin. Use self.state.reshape if goal is consensus.
        ...
        diff = self.state.T.reshape((self.N,self.ns,1)) - np.zeros((1,self.ns,self.N))
...
class CentralizedConvergeContEnv(ConvergeContEnv):
    """Custom Environment that follows gym interface"""
    metadata = {'render.modes': ['human']}
