        ...
        with torch.no_grad():
            if self.centralized:
                if self.rand_modeA == NO_RAND:
                    return self.netA(state.view(1,-1,self.N)).squeeze().view(self.N,-1)#.detach().numpy()
...
#                         print(distrb, distrb.shape)
                        ...
                        return torch.clamp( distrb.sample(), 
                                           self.action_range[0], self.action_range[1] ).squeeze().view(self.N,-1)#.detach().numpy()
                        # Adding the .view() at the end could help prevent problem when N=1
            else:
                if self.rand_modeA == NO_RAND:
                    return self.netA(state.view(1,-1,self.N)).squeeze()#.detach().numpy()
                elif self.rand_modeA == GAUSS_RAND:
...
#                 print(distrb_params.shape, nn.functional.softplus( distrb_params[:,:,self.na:] ) )
                ...
                # This pred_probs would be of size (B, self.N, self.na), as is the shape of distrb's mean and sigma. 
                # It would represent the log likelihood of each action, if it were generated by the Actor's predicted 
                # distribution for that specific scenario by the current model. 
                # Several lines below, this log_prob matrix is multiplied by the Critic's predictions, shaped (B,1,1). By
                # taking the mean of the product, we effectively adds up the log probability products for each batch element.
                # This should be equivalent to summing up all the log probabilities for each action taken (and obtaining 
                # the real overall log probability), and then multiplying it with whatever value the Critic gives.
            ...
            if self.rand_modeA == NO_RAND:
                ...
                if not_use_rand:
                    ...
                    if self.centralized:
                        ...
                        return torch.clamp( distrb.sample(), self.action_range[0], self.action_range[1] ).squeeze().view(self.N,-1)#.detach().numpy()
                    else:
                        distrb = self.netA(state.view(1,-1,self.N)).squeeze()
                        distrb = torch.distributions.Normal(
                            distrb[:self.na],
